    ...
    try {
        foreach ($uploadGroups as $field => $label) {
            ...
            foreach ($_FILES[$fileKey]['name'] ?? [] as $i => $name) {
                ...
                $mime = is_file($tmp) && function_exists('mime_content_type') ? (mime_content_type($tmp) ?: '') : '';
                ...
                if (move_uploaded_file($tmp, $uploadDir . '/' . $safe)) {
                    ...
                    $payload['upload_details'][$field][] = (string)$name;
                }
            }
        }
    } catch (Throwable $e) {
        $error = 'One upload could not be processed. Try converting the file to PDF or uploading fewer files at once.';
    }
    if (!$error) {
        try {
...
<section class="result-panel smart-file-brain">
  ...
  <div class="brain-grid">
    ...
    <?php if (!empty($brain['document_review'])): ?><article><h3>Document-by-document reading notes</h3><?= vm_list_html($brain['document_review']) ?></article><?php endif; ?>
    <?php if (!empty($brain['detailed_explanations'])): ?><article><h3>Detailed cleanup explanation</h3><?= vm_list_html($brain['detailed_explanations']) ?></article><?php endif; ?>
...
<form class="card form-card" id="review-form" method="post" enctype="multipart/form-data">
  ...
  <div class="upload-brain-grid" data-dropzone-grid>
    <label class="drop-upload-zone">Passport / ID pages <span>Drag files here or click to choose</span><input type="file" name="documents_passport[]" multiple accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"><small data-file-summary>No files selected</small></label>
    <label class="drop-upload-zone">Application form / portal confirmation <span>Drag files here or click to choose</span><input type="file" name="documents_form[]" multiple accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"><small data-file-summary>No files selected</small></label>
    <label class="drop-upload-zone">Bank / funding proof <span>Drag files here or click to choose</span><input type="file" name="documents_money[]" multiple accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"><small data-file-summary>No files selected</small></label>
    <label class="drop-upload-zone">Employment / business proof <span>Drag files here or click to choose</span><input type="file" name="documents_work[]" multiple accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"><small data-file-summary>No files selected</small></label>
    <label class="drop-upload-zone">Invitation / admission / host proof <span>Drag files here or click to choose</span><input type="file" name="documents_invitation[]" multiple accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"><small data-file-summary>No files selected</small></label>
